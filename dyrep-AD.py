...
class DyRep(nn.Module):
    def __init__(self,
                 ...
                 freq=False,
                 verbose=False,
                 information_progation = "node",
                 information_time = "new",
                 remove_v = True,
                 unblance = 0,
                 attention_mode = "no",
                 aggregation = True,
        ...
        self.unblance = unblance # 0: balance 1: random mask some neighbors during aggregation stage 2. random mask some neighbors during diffusion stage
                                 # 3: random mask neighbors during aggregation and diffusion stage independent 4: random mask neighbors during aggregation and diffusion
        self.aggregation = aggregation
        self.attention_mode = attention_mode
        self.information_progation = information_progation
        ...
        if self.model != 'dyrep':
            ...
            # 1-st order update
            if self.information_progation == "node" or self.information_progation == "delta":
                self.W_h2 = nn.Linear(in_features=n_hidden+n_in, out_features=n_hidden, bias=False)
            if self.information_progation == "edge":
                self.W_hu = nn.Linear(in_features=n_hidden + n_in, out_features=n_hidden)
                self.W_hv = nn.Linear(in_features=n_hidden + n_in, out_features=n_hidden)
            if self.information_progation == "diff":
                self.W_u_hatu = nn.Linear(in_features=n_hidden*2, out_features=n_hidden)
    ...
    def intensity_rate_lambda(self, z_u, z_v, k):
        ...
        Lambda_np = Lambda.clone().detach().cpu().numpy()
        return Lambda
    ...
    def update_node_embed(self, prev_embed, node1, node2, time_delta_uv, k):
        ...
        for c, (v, u, delta_t) in enumerate(zip([node1, node2], [node2, node1], time_delta_uv)):  # i is the other node involved in the event
            ...
            for rel in range(self.n_assoc_types):
                ...
                if N_neighb > 0:  # node has no neighbors
                    neighbors_index = list(Neighb_u.nonzero(as_tuple=True)[0])
                    if self.unblance == 1 or self.unblance == 3 or self.unblance == 4:
                        masked_index = np.random.choice(neighbors_index, size=int(np.around(len(neighbors_index) * 0.2)))
                        for m_index in masked_index:
                            Neighb_u[m_index] = 0
                        if self.unblance == 4:
                            neighbors_used[c] = [inx for inx in neighbors_index if inx not in masked_index]
        ...
        if self.information_progation == "node":
            message_passing = self.W_h2(torch.sigmoid(h1 + h2 + h3)) if self.aggregation else self.W_h2(torch.sigmoid(h2 + h3))
        elif self.information_progation == "delta":
            message_passing = self.W_h2(torch.sigmoid(h1 + h2 + h3) - node_embed[[node1, node2]])
        elif self.information_progation == "edge":
            new_node_embed = torch.sigmoid(h1 + h2 + h3)
            message_passing = torch.sigmoid(self.W_hu(new_node_embed[0]) + self.W_hv(new_node_embed[1]))
        elif self.information_progation == "diff":
            message_passing = self.W_u_hatu(torch.cat((torch.sigmoid(h1 + h2 + h3), node_embed[[node1, node2]]), dim=1))

        z_new[[node1, node2], :] = torch.sigmoid(h1 + h2 + h3) if self.aggregation else torch.sigmoid(h2 + h3)

        ## Here consider random walk. or we can consider 2-hop update first.
        # 1-st first
        for rel in range(self.n_assoc_types):
            for c, (u, v) in enumerate(zip([node1, node2], [node2, node1])):
                if not self.latent_graph:
                    neighbors_u = self.A[u, :, rel] > 1e-7
                else:
                    neighbors_u = self.S[u, :, rel] > 0
                if self.remove_v:
                    neighbors_u[v] = 0  # dont update v (another node in event)
                neighbors_u_torch = neighbors_u
                n_neighb = torch.sum(neighbors_u).item()
                neighbors_u = list(neighbors_u.nonzero(as_tuple=True)[0])
                if self.unblance == 2 or self.unblance == 3:
                    neighbors_u = list(np.random.choice(neighbors_u, size=np.int(np.around(len(neighbors_u) * 0.8))))

                if self.unblance == 4:
                    neighbors_u = neighbors_used[c]
                    n_neighb = len(neighbors_u)
                if n_neighb > 0:
                    ## using attention?
                    if self.attention_mode == "mode1":
                        q_ui = torch.exp(self.S[u, neighbors_u_torch, rel]).view(n_neighb, 1)
                        q_ui = q_ui / (torch.sum(q_ui) + 1e-7)
                    if self.attention_mode == "mode2":
                        q_ui = torch.exp(self.S[neighbors_u_torch, :, rel]).view(n_neighb, self.N_nodes)
                        q_ui = q_ui[:,u].view(1, n_neighb)/(torch.sum(q_ui, dim=1) + 1e-7).view(1, n_neighb)
                        q_ui = q_ui.view(n_neighb, 1)
                    # q_ui = torch.exp(self.S[u, Neighb_u, rel]).view(N_neighb, 1)
                    # q_ui = q_ui / (torch.sum(q_ui) + 1e-7)
                    if self.information_progation == "node" or self.information_progation == "diff" or self.information_progation == "delta":
                        message_passing_c = message_passing[c]
                        #message_passing_c = self.W_h2(message_passing[c])
                    elif self.information_progation == "edge":
                        message_passing_c = message_passing
                    #message_passing = q_ui * message_passing
                    if self.attention_mode != "no":
                        z_new[neighbors_u, :] = torch.sigmoid(z_new[neighbors_u, :] + q_ui * message_passing_c)
                    else:
                        z_new[neighbors_u, :] = torch.sigmoid(z_new[neighbors_u, :] + message_passing_c)
        ...
        for it, k in enumerate(event_types):
            ...
            if self.generate:
                if self.encoder is not None:
                    ...
                    self.S = self.S / (torch.sum(self.S, dim=1, keepdim=True) + 1e-6)
